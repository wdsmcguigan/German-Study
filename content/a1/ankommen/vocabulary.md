# Wortschatz – Ankommen

## Kursaktivitäten (Classroom Actions)

| Deutsch | Englisch |
|---------|----------|
| an·schauen / an·sehen | to look at |
| hören | to listen |
| noch einmal | once more |
| an·kreuzen | to tick / mark with a cross |
| zu·ordnen | to match / assign |
| ergänzen | to fill in / complete |
| machen | to do / make |
| sprechen | to speak |
| lesen | to read |
| spielen | to play |
| suchen | to look for / search |
| zeigen | to show |
| fragen | to ask |
| markieren | to mark / highlight |
| raten | to guess |
| sagen | to say |
| notieren | to note down |
| vergleichen | to compare |
| schreiben | to write |
| variieren | to vary |
| erzählen | to tell / narrate |
| zeichnen | to draw |
| verbinden | to connect |
| antworten | to answer |

## Wichtige Nomen (Key Nouns)

| Deutsch | Artikel | Plural | Englisch |
|---------|---------|--------|----------|
| Bild | das | Bilder | picture / image |
| Foto | das | Fotos | photo |
| Gespräch | das | Gespräche | conversation |
| Frage | die | Fragen | question |
| Antwort | die | Antworten | answer |
| Wort | das | Wörter | word |
| Lektion | die | Lektionen | lesson |
| Person | die | Personen | person |
| Kurs | der | Kurse | course |

## Unterrichtsanweisungen (Classroom Instructions)

| Anweisung |
|-----------|
| Sehen Sie die Fotos / Bilder an. |
| Hören Sie. |
| Hören Sie noch einmal. |
| Kreuzen Sie an. |
| Ordnen Sie zu. |
| Ergänzen Sie. |
| Sprechen Sie im Kurs. |
| Lesen Sie. |
| Lesen Sie die Gespräche. |
| Spielen Sie die Gespräche im Kurs. |
| Suchen Sie. |
| Zeigen Sie. |
| Fragen Sie im Kurs. |
| Markieren Sie. |
| Hören Sie und sprechen Sie nach. |
| Raten Sie Wörter aus der Lektion. |
| Was meinen Sie? |
| Notieren Sie. |
| Vergleichen Sie. |
| Schreiben Sie Gespräche. |
| Variieren Sie. |
| Erzählen Sie. |
| Arbeiten Sie zu zweit. |
| Zeichnen Sie. |
| Hören Sie und verbinden Sie. |
| Schreiben Sie Fragen und Antworten. |
